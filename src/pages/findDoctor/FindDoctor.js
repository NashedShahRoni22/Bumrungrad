...
export default function FindDoctor() {
  ...
  useEffect(() => {
    fetch(
      "http://api.bumrungraddiscover.com/api/get/specialty"
    )
      .then((res) => res.json())
  ...
  useEffect(() => {
    if (speacility) {
      fetch(
        `http://api.bumrungraddiscover.com/api/get/selected/sub/specialty/${speacility}`
      )
        .then((res) => res.json())
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      ...
      // Create the base URL
      const baseUrl =
        "http://api.bumrungraddiscover.com/api/search/doctor";
        ...
        <div className="mx-5 p-5 md:container md:mx-auto relative">
          ...
          {doctors?.length > 0 ? (
            <div>
              ...
              <div className="grid place-items-center  md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-5 my-5">
                {doctors.slice(0, showData).map((d, i) => (
                  <div
                    className="shadow hover:shadow-xl duration-300 ease-linear flex flex-col gap-5 w-full md:h-[520px] rounded-xl "
                    key={i}
                  >
