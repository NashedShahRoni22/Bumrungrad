...
export default function FindDoctor() {
  ...
  const handaleAddData = (num) => {
    const newShowData = showData + num
  ...
  const currentDoctors = doctors?.slice(0, showData)
  //get speacilities
  useEffect(() => {
  ...
  return (
    <section className='min-h-screen'>
      ...
      {loader ? (
        ...
        <div className='mx-5 p-5 md:container md:mx-auto relative'>
          <button
            ...
            className='p-2 md:p-4 absolute bottom-[-32px] right-2 bg-blue border border-blue rounded-full'
          >
            <FaArrowUp className='text-xl md:text-3xl text-white' />
          </button>
          {doctors?.length > 0 ? (
            <div>
      ...
      <div className='flex justify-center items-center gap-2 md:gap-4 mt-8 '>
        <button
          onClick={() => handalelessData(15)}
          className='bg-blue text-sm md:text-base text-white px-2 md:px-6 py-2'
          disabled={showData === 15}
        >
          View less
        </button>
        <button
          onClick={() => handaleAddData(15)}
          className='bg-blue text-sm md:text-base text-white   px-2 md:px-6 py-2'
